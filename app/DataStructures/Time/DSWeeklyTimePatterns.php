...
class DSWeeklyTimePatterns implements
    \Iterator,
    \Countable,
    \ArrayAccess,
    IClonable,
    ...
    Arrayable
{
    ...
    protected DSTimePatterns $Monday;

    protected DSTimePatterns $Tuesday;

    protected DSTimePatterns $Wednesday;

    protected DSTimePatterns $Thursday;

    protected DSTimePatterns $Friday;

    protected DSTimePatterns $Saturday;

    protected DSTimePatterns $Sunday;
    ...
    {
        $new = new static(array_key_first($resultOfToArrayMethod));
        ...
        foreach ($resultOfToArrayMethod as $weekDay => $dateTimePeriods) {
            ...
            $dsDateTimePeriods = DSTimePatterns::toObject($dateTimePeriods);

            $new[$weekDay] = $dsDateTimePeriods;
        }
    ...
    private function validateValue(DSTimePatterns $value): void
    {
        return;
    }
    ...
    {
        $new = new static($this->startingDay);

        foreach (static::$weekDays as $weekDay) {
            $new[$weekDay] = $this->{$weekDay}->cloneIt();
        }
